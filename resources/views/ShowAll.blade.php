<x-layout>
    ...
    <div class="question_grid">
      ...
      <span class="qgrid ">{{ $record->id }}</span> 
      <span class="qgrid ">{{ $record->question }}</span>
      <span class="qgrid ">{{ $record->TrueAnswer }}</span>
      <span class="qgrid ">{{ $record->FalseAnswer1 }}</span>
      <span class="qgrid ">{{ $record->FalseAnswer2 }}</span> 
      <span class="qgrid ">{{ $record->FalseAnswer3 }}</span>   
      <span><a href="{{ url('/edit/'.$record->id) }}" class="qgrid1" >Modyfikuj </a></span>
      @endforeach
      </div>  
